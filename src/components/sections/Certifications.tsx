...
export default function Certifications({ themeMode = 'dark' }: Props) {
  ...
  return (
    <section id="certs" className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`px-2.5 py-1 rounded-full text-xs font-mono border ${
                      isLight ? 'bg-blue-50 border-blue-200 text-blue-900 font-medium' : 'bg-black/40 border-white/10 text-slate-400'
                    }`}>
                      {cert.year}
                    </span>
                  </div>
                    ...
                    {cert.skills.map((skill) => (
                      <span
                        ...
                        className={`px-2 py-0.5 rounded text-[10px] font-mono border ${
                          isLight ? 'bg-blue-50 border-blue-200 text-blue-900' : 'bg-white/5 text-slate-300 border-white/5'
                        }`}
                      >
                        {skill}
                      </span>
