...
export default function Experience({ themeMode = 'dark' }: Props) {
  ...
  const allTags = Array.from(new Set(EXPERIENCES.flatMap((e) => e.tech)));

  const filteredExperiences = selectedTag
    ? EXPERIENCES.filter((e) => e.tech.includes(selectedTag))
    : EXPERIENCES;
      ...
      <div className="space-y-10">
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-2 pt-2 font-mono text-xs">
            <span className={isLight ? 'text-slate-500 mr-1' : 'text-slate-400 mr-1'}>Filter by Tech:</span>
            <button
        ...
        <div className={`relative pl-6 sm:pl-8 border-l-2 space-y-12 ${
          ...
          {filteredExperiences.map((exp, idx) => (
            ...
            >
              ...
              <div className={`liquid-glass rounded-2xl p-6 border transition-all shadow-xl space-y-4 ${
                isLight ? 'bg-white/90 border-blue-200 text-slate-800' : 'border-white/10 text-slate-300'
              }`}>
                {/* Header Meta */}
                <div className={`flex flex-wrap items-start justify-between gap-2 border-b pb-4 ${
                  ...
                  <div>
                    ...
                    <p className={`font-medium text-sm ${isLight ? 'text-blue-900' : 'text-slate-300'}`}>
                      {exp.company}
                    </p>
                  </div>

                  <div className={`text-right font-mono text-xs flex items-center gap-1 px-3 py-1.5 rounded-lg border ${
                    isLight ? 'bg-blue-50 border-blue-200 text-blue-900' : 'bg-black/30 border-white/5 text-slate-400'
                  }`}>
                ...
                <ul className={`space-y-2.5 text-sm leading-relaxed font-sans ${
                  isLight ? 'text-slate-700' : 'text-slate-300'
                }`}>
                  {exp.bullets.map((bullet, bIdx) => (
                    <li key={bIdx} className="flex items-start gap-2.5">
                      <span className="text-blue-500 mt-1 select-none font-mono">--</span>
                      <span>{bullet}</span>
                    </li>
                  ))}
                </ul>
