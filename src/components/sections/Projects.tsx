        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {PROJECTS.map((proj, idx) => (
            ...
            >
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className={`px-2.5 py-1 rounded-full text-[11px] font-mono border ${
                    isLight ? 'bg-blue-50 border-blue-200 text-blue-900 font-medium' : 'bg-white/5 border-white/10 text-slate-300'
                  }`}>
                    {proj.type}
                  </span>
                </div>
                ...
                {/* Title & Description */}
                <div>
                  <h3 className={`text-xl font-bold font-sans tracking-tight transition-colors ${
                    isLight ? 'text-slate-900 group-hover:text-blue-600' : 'text-white group-hover:text-blue-400'
                  }`}>
                    {proj.title}
                  </h3>
                  <p className={`text-sm leading-relaxed mt-2 font-sans font-normal ${
                ...
                {proj.metrics && (
                  <div className="grid grid-cols-3 gap-2 pt-1 font-mono text-[10px]">
                    {proj.metrics.map((m, mIdx) => (
                      <div key={mIdx} className={`p-2 rounded-lg border text-center ${
                        isLight ? 'bg-blue-50/80 border-blue-200' : 'bg-black/40 border-white/5'
                      }`}>
                        <span className={isLight ? 'text-slate-500 block' : 'text-slate-400 block'}>{m.label}</span>
                        <span className="text-blue-500 font-bold">{m.value}</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div className={`space-y-4 pt-4 border-t ${isLight ? 'border-blue-100' : 'border-white/10'}`}>
                {/* Tech Chips */}
                  ...
                  {proj.tags.map((tag) => (
                    <span
                      ...
                      className={`px-2 py-0.5 rounded border text-[11px] ${
                        isLight ? 'bg-blue-50 border-blue-200 text-blue-900' : 'bg-white/5 border-white/5 text-slate-300'
                      }`}
                    >
                      {tag}
                    </span>
