...
import { Mail, Github, Linkedin, Send, Check, Copy, Terminal } from 'lucide-react';
    ...
    <section id="contact" className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      <div className="space-y-10">
        ...
        >
          ...
          <p className={`text-base max-w-2xl font-sans leading-relaxed ${
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-5 space-y-4">
            ...
            <div className={`liquid-glass rounded-2xl p-5 border space-y-3 font-mono text-xs ${
              isLight ? 'bg-white/90 border-blue-200 text-slate-800' : 'bg-black/60 border-white/10 text-slate-300'
            }`}>
              <div className={`flex items-center justify-between border-b pb-2 ${
                isLight ? 'border-blue-100 text-slate-600' : 'border-white/10 text-slate-400'
              }`}>
              ...
              <div className={`space-y-1 text-[11px] ${isLight ? 'text-slate-600' : 'text-slate-400'}`}>
                <p>64 bytes from vian.dev (104.21.80.12): icmp_seq=1 ttl=58 time=12.1 ms</p>
                <p>64 bytes from vian.dev (104.21.80.12): icmp_seq=2 ttl=58 time=11.8 ms</p>
                <p>64 bytes from vian.dev (104.21.80.12): icmp_seq=3 ttl=58 time=12.4 ms</p>
                <p className="text-blue-600 dark:text-blue-300 pt-1">--- vian.dev ping statistics --- 0% packet loss, avg rtt = 12.1ms</p>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-7">
            <div className={`liquid-glass rounded-2xl p-6 border shadow-2xl space-y-5 ${
              isLight ? 'bg-white/90 border-blue-200' : 'border-white/10'
            }`}>
              <div className={`border-b pb-3 flex items-center justify-between font-mono text-xs ${
                isLight ? 'border-blue-100 text-slate-600' : 'border-white/10 text-slate-400'
              }`}>
                <span>cat &gt; /dev/messages/vian.msg</span>
                <span className="text-blue-500 font-semibold">SMTP Message Handler</span>
              </div>

              <form action={web3formAction} method="POST" onSubmit={handleSubmit} className="space-y-4 font-mono text-xs">
                {/* Web3form hidden fields placeholder */}
                <input type="hidden" name="access_key" value="YOUR_WEB3FORM_ACCESS_KEY" />

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className={isLight ? 'text-slate-700 font-semibold' : 'text-slate-400 font-semibold'}>Your Name:</label>
                    <input
                      type="text"
                      name="name"
                      required
                  ...
                  <div className="space-y-1.5">
                    <label className={isLight ? 'text-slate-700 font-semibold' : 'text-slate-400 font-semibold'}>Your Email:</label>
                    <input
                      type="email"
                      name="email"
                      required
                ...
                <div className="space-y-1.5">
                  <label className={isLight ? 'text-slate-700 font-semibold' : 'text-slate-400 font-semibold'}>Message Payload:</label>
                  <textarea
                    required
                    name="message"
                    rows={4}
